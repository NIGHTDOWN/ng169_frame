...
class Upfile
{
  ...
  public function __construct($conf = null)
  {
    if ($conf != null) {
      ...
      $confdir = preg_replace( "/\./",'',$conf['upfilepath']);//避免后台夸目录上传
  ...
  public function upload($name, $module = '', $path = '', $params = array())
  {
    ...
    if (empty($path)) {
      $path = self::FILE_PATH . $datedir;
    }
    else {
      $path = self::FILE_PATH .'/'.$conf['upfilepath'] . '/' . $path. $datedir;
    }
    ...
    if ($result == false) {
      ...
      $uploadInfo['source'] = 'http://'.$_SERVER['SERVER_NAME'].'/'.$uploadInfo['source'];
  ...
  {
    $temp = array();
    $temp['maxSize'] = (isset($params['maxSize'])) ? (int)$params['maxSize'] : $this->
    defaultMaxSize;
  ...
  {
    if (!in_array(strtolower($uploadType), $this->params['allowFileType']) ||
  ...
  private function checkIsUploadFile($tmpName)
  {
    ...
    elseif (function_exists('is_uploaded_file') && !is_uploaded_file($tmpName) && !
      is_uploaded_file(str_replace('\\\\', '\\', $tmpName))) {
      return false;
    }
    else {
      return true;
  ...
  private function escapeStr($string)
  {
    $string = str_replace(array(
        ...
        "%00",
        "\r"), '', $string);
    $string = preg_replace(array('/[\\x00-\\x08\\x0B\\x0C\\x0E-\\x1F]/',
        '/&(?!(#[0-9]+|[a-z]+);)/is'), array('','&amp;'), $string);
