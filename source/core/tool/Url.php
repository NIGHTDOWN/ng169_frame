...
class Url
{
    ...
    function get_url_dynamic(
        ...
        $url = self::getadd() . '/' . $file . $m . 'c=' . $mod . $fh . 'a=' . $action . $param;
        return $url;
    }
    /**
     * 返回伪静态url
    ...
    function get_url_static(
        ...
        $url = trim(self::getadd()  . '/' . $group . $mod . $action . $param, '/') . $pre;
        return $url;
    }
    public static function getadd()
    {
        ...
        if ($_SERVER['SERVER_PORT'] == '80') {
            $add = self::gethttp() . $_SERVER["SERVER_NAME"];
        } else {
            $add
                = self::gethttp() . $_SERVER["SERVER_NAME"] . ":" . $_SERVER['SERVER_PORT'];
        }
        return $add;
    }
    public static
